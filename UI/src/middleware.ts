...
export function middleware(request: NextRequest) {
  ...
  const isAuthPage = ['/login', '/sign-up', '/forgot-password', '/reset-password'].includes(pathname);
  ...
  if (accessToken) {
    ...
    if (decoded && !decoded.isVerified) {
      ...
      const isAllowed = allowedForUnverified.some(prefix => pathname.startsWith(prefix));
...
export const config = {
  matcher: [
    '/((?!_next|favicon.ico|logo.png|images|api|.*\\..*).*)',
  ],
};
